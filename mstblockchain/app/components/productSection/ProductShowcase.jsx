...
const logos = [
  {
    ...
    title: "Bridge Key",
    description: "Everything You Need in One Wallet. Bridgekey combines powerful features with elegant simplicity.",
    stats: { users: "85K+", downloads: "120K", reliability: "99.9%" }
  },
  {
    src: "/assets/logo-buddy.svg",
    title: "MST Buddy",
    description: "AI-powered assistant platform acting as a 24/7 smart guide for the MST ecosystem.",
    stats: { queries: "1.2M", accuracy: "98%", active_nodes: "9+" }
  },
  {
    src: "/assets/logo-masterstroke.svg",
    title: "MST Academy",
    description: "Educational gateway for blockchain mastery, certification, and Web3 developer tours.",
...
const LogoFlipSection = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      setActiveIndex(Math.min(logos.length - 1, Math.max(0, Math.round(rawIndex))));
        ...
        <div className="container mx-auto px-6 lg:px-16 relative z-10">
          <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
            ...
            <div className="w-full lg:w-1/2 flex justify-center perspective-container">
              ...
              >
                <img src={current.src} alt={current.title} className="max-w-[70%] object-contain" 
                     style={{ transform: Math.floor(rotateY / 180) % 2 === 0 ? "" : "scaleX(-1)" }} />
              </motion.div>
            </div>

            {/* RIGHT → TEXT & LIVE STATES */}
            <div className="w-full lg:w-1/2">
              <AnimatePresence mode="wait">
                ...
                >
                  <h1 className="text-5xl font-black uppercase mb-4">
                    <span className="bg-black text-white px-2 mr-2">{firstWord}</span>
                    <span className="text-red-600">{secondWord}</span>
                  </h1>
                  <p className="text-xl text-gray-600 mb-8 max-w-md">{current.description}</p>

                  {/* LIVE DATA GRID */}
                  <div className="grid grid-cols-3 gap-4 mb-10">
                    {Object.entries(current.stats).map(([key, value]) => (
                      <div key={key} className="border-l-2 border-red-600 pl-4">
                        <p className="text-xs uppercase tracking-widest text-gray-400 font-bold">{key.replace('_', ' ')}</p>
              ...
              <div className="flex gap-3 justify-center lg:justify-start">
                {logos.map((_, i) => (
                  <div key={i} className={`w-3 h-3 rounded-full transition-all ${i === activeIndex ? "bg-red-600 w-8" : "bg-gray-300"}`} />
                ))}
              </div>
            </div>
